...
//        echo '<br>';
     ...
     * @param User $user
     */
    public function timeShiftForMonth(Holyday $holyday, User $user, Requests\Validator $request)
    {
        $reportForUsers = [];
        $reportForMonth = [];
        $reportForDays = [];
...
//                echo 'day='.$day;
               ...
               $years =  sprintf("%04d", $year);
                ...
                /*if($day<10 && $month<10){
                    ...
                    $reportForDays[$day] = $this->getTimesheet($holyday,$user,'0'.$day,'0'.$month,$year);
                }elseif($day<10 && $month>=10){

                    $reportForDays[$day] = $this->getTimesheet($holyday,$user,'0'.$day,$month,$year);
                }elseif($day>=10 && $month<10){

                    $reportForDays[$day] = $this->getTimesheet($holyday,$user,$day,'0'.$month,$year);
                }*/
...
//                echo '<br>';
            ...
            /*echo '<pre>';
            var_dump($reportForDays) ;

            echo '</pre>';*/
//                        $this->getTimesheet($holyday,$user,'08','03',2016);
//            $reportForUsers[$user->id] = [$user,$reportForDays];
...
//        dd($user);
//        dd( $reportForUsers);
//        dd($reportForUsers[120][1]) ;

        return view('default.admin.timesheet.time-report',compact('reportForUsers','locale','defaultLocale','month','year'));
    }
...
//        dd($reportForDays);
        ...
        return view('default.admin.timesheet.user-daily-time-report',compact('reportForUsers','year','month','locale','defaultLocale'));
