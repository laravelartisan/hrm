...
class GenderController extends Controller
{
        ...
        foreach ($this->gender->translatedAttributes as $field) {
            foreach(config('app.locales') as $locale => $value){
                if($validatedRequest->get($field.'_'.$locale)){
                    $this->gender->translateOrNew($locale)->{$field} =ucwords($validatedRequest->get($field.'_'.$locale));
                }
            }
        }
        $this->gender->status = ucwords($validatedRequest->get('status'));
    ...
    public function editGender($id, Requests\Validator $validatedRequest)
    {
        ...
        foreach ($genderToEdit->translatedAttributes as $field) {
            foreach(config('app.locales') as $locale => $value){
                if($validatedRequest->get($field.'_'.$locale)){
                    $genderToEdit->translateOrNew($locale)->{$field} =ucwords($validatedRequest->get($field.'_'.$locale));
                }
            }
        }
        $genderToEdit->status = ucwords($validatedRequest->get('status'));
    ...
    public function viewGender($id)
    {
        ...
        return view('default.admin.gender.view',compact('genderToView'));
